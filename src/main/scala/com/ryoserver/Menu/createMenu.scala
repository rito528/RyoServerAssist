...
object createMenu {
  ...
  def menu(p:Player,ryoServerAssist: RyoServerAssist): Inventory = {
    ...
    inv.setItem(0,getItem(Material.CHEST_MINECART,s"${GREEN}運営からのガチャ券を受け取ります。",List(s"${GRAY}クリックで受け取ります。").asJava))
    ...
    inv.setItem(11,getItem(Material.LAVA_BUCKET,s"${GREEN}ゴミ箱を開きます。",List(s"${GRAY}クリックで開きます。",s"${RED}${BOLD}取扱注意！").asJava))
    ...
    inv.setItem(20,getItem(Material.SHULKER_BOX,s"${GREEN}ネオスタックを開きます。",List(s"${GRAY}クリックで開きます").asJava))
    inv.setItem(22,getGachaItem(Material.HONEY_BOTTLE,s"${GREEN}ガチャ特等アイテム交換画面を開きます。",List(s"${GRAY}クリックで開きます。").asJava))
    inv
  }
}
