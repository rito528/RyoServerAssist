...
class createData(ryoServerAssist: RyoServerAssist) {
  ...
  def createPlayerData(p:Player): Unit = {
    ...
    if (!table_rs.next()) sql.executeSQL("CREATE TABLE Players(UUID Text,lastLogin DATETIME,loginDays INT,consecutiveLoginDays INT,lastDistributionReceived INT);")
    val user_rs = sql.executeQuery(s"SELECT UUID FROM Players WHERE UUID='${p.getUniqueId.toString}';")
    if (!user_rs.next()) sql.executeSQL(s"INSERT INTO Players (UUID,lastLogin,loginDays,consecutiveLoginDays,lastDistributionReceived) " +
      s"VALUES ('${p.getUniqueId}',NOW(),1,1,(SELECT last_insert_id() Distribution));")
    sql.close()
  }
