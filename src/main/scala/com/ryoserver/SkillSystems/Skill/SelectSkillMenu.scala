...
class SelectSkillMenu(ryoServerAssist: RyoServerAssist) extends Menu {
  ...
  def openMenu(player: Player): Unit = {
    ...
    setItem(4, 4, if (check.isOpened(SkillNames(5), p)) Material.ENCHANTED_GOLDEN_APPLE else Material.BEDROCK, effect = false,
      s"${GREEN}[基本スキル]${SkillNames(5)}", List(s"${GRAY}再生能力1の効果が付与されます。",
        ...
        s"${GRAY}スキルポイントコスト:300"))
    setItem(6, 4, if (check.isOpened(SkillNames(6), p)) Material.ELYTRA else Material.BEDROCK, effect = true,
    ...
    setItem(8, 4, if (check.isOpened(SkillNames(7), p)) Material.ENDER_EYE else Material.BEDROCK, effect = true,
      ...
      List(s"${GRAY}暗視の効果が付与されます。",
        ...
        s"${GRAY}スキルポイントコスト:600"))
    setItem(2, 6, if (check.isOpened(SkillNames(8), p)) Material.LAVA_BUCKET else Material.BEDROCK, effect = true,
      ...
      List(s"${GRAY}耐火の効果が付与されます。",
        ...
        s"${GRAY}スキルポイントコスト:600"))
    setItem(4, 6, if (check.isOpened(SkillNames(9), p)) Material.WATER_BUCKET else Material.BEDROCK, effect = true,
  ...
  def motion(player: Player, index: Int): Unit = {
    ...
    index match {
      ...
      case 52 => new RyoServerMenu1(ryoServerAssist).menu(p)
      case _ =>
    }
  }
