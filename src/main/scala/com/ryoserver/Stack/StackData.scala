...
class StackData(ryoServerAssist: RyoServerAssist) {
  ...
  def getSetItems(uuid:String): Array[ItemStack] = {
    val sql = new SQL(ryoServerAssist)
    val checkTable = sql.executeQuery("SHOW TABLES LIKE 'Stack';")
    if (!checkTable.next()) {
      sql.executeSQL("CREATE TABLE Stack(id INT AUTO_INCREMENT,UUID TEXT,item TEXT,PRIMARY KEY(`id`))")
      sql.close()
      Array.empty
    } else {
      val rs = sql.executeQuery(s"SELECT item FROM Stack WHERE UUID='$uuid';")
